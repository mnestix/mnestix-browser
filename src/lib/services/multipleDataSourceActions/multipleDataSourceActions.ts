...
export async function performSearchAasFromAllRepositories(searchInput: string): Promise<RepoSearchResult[]> {
    const searcher = MultipleDataSourceActions.create();
...
export class MultipleDataSourceActions {
    private constructor(
    ...
    static create(): MultipleDataSourceActions {
        ...
        return new MultipleDataSourceActions(repositoryClient, submodelRepositoryClient, fetch, log);
    }
    ...
    static createNull({
        ...
        log = null,
    }: NullableSearchSetupParameters = {}): MultipleDataSourceActions {
        ...
        return new MultipleDataSourceActions(
